...
export const PlacementResultDashboard: React.FC<PlacementResultDashboardProps> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-white rounded-xl shadow-none border border-blue-100">
        ...
        <div className="flex items-center gap-3">
          <label className="flex flex-wrap items-center gap-2 text-sm font-bold text-gray-700 bg-amber-50 px-3.5 py-2 rounded-lg border border-amber-200 cursor-pointer hover:bg-amber-100 transition">
            <input
              type="checkbox"
      ...
      {swapCandidate && !isReadOnly && (
        <div className="p-3 sm:p-4 bg-blue-50 border border-blue-300 rounded-xl flex flex-wrap gap-3 items-center justify-between text-sm text-[#172b4d]">
          <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="px-3 py-1.5 bg-white border border-slate-200 rounded-lg font-semibold hover:bg-slate-50 text-slate-600"
          >
            취소
          </button>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-none border border-blue-100 overflow-hidden">
        <div className="p-3 sm:p-4 border-b border-blue-100 flex justify-between items-center bg-blue-50/50 overflow-x-auto">
          <div className="flex gap-2 min-w-max">
            <button
              ...
              className={`px-3 py-2 rounded-full text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 whitespace-nowrap ${
                activeTab === 'ALL' ? 'bg-blue-600 text-white shadow-none' : 'bg-white text-[#354c75] ring-1 ring-inset ring-blue-100 hover:bg-blue-50'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-2 rounded-full text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 whitespace-nowrap flex items-center gap-1.5 border ${
              ...
              result.activeClassNames.map(name => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-2 rounded-full text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 whitespace-nowrap flex items-center gap-1.5 ${
                      activeTab === name ? 'bg-blue-600 text-white shadow-none' : 'bg-white text-[#354c75] ring-1 ring-inset ring-blue-100 hover:bg-blue-50'
                    }`}
                  >
                    <span>{name}반</span>
              ...
              oldClassNumbers.map(classNum => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-2 rounded-full text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 whitespace-nowrap ${
                      activeTab === String(classNum) ? 'bg-blue-600 text-white shadow-none' : 'bg-white text-[#354c75] ring-1 ring-inset ring-blue-100 hover:bg-blue-50'
                    }`}
                  >
          ...
          {onReset && (
            <div className="flex gap-2 ml-4">
              <button onClick={onReset} title="재배정 설정으로 돌아가기" className="p-2 rounded-lg bg-white border border-blue-100 text-blue-600 hover:bg-blue-50 cursor-pointer">
          ...
          {activeTab === 'SPECIAL_DASHBOARD' ? (
            <div className="space-y-4">
              <div className="bg-white border border-blue-100 rounded-xl p-4">
                <h4 className="text-base font-black text-amber-950 flex flex-wrap items-center gap-2">
                ...
                <div className="mt-4 overflow-x-auto bg-white rounded-xl border border-blue-100 shadow-none">
                  <table className="w-full text-xs text-center border-collapse">
                    <thead>
                      <tr className="bg-blue-50 border-b border-blue-100 text-[#172b4d] font-bold">
                        <th className="py-2.5 px-3 border-r border-blue-100">구분</th>
                        {result.activeClassNames.map(cls => (
                          <th key={cls} className="py-2.5 px-3 border-r border-blue-100">{cls}반</th>
                        ))}
                        <th className="py-2.5 px-3 bg-blue-100/60 font-bold">합계</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-blue-50 text-slate-700">
                      <tr>
                        <td className="py-2 px-3 font-bold bg-slate-50 border-r border-blue-100 text-left">총 배정 인원</td>
                        {result.activeClassNames.map(cls => (
                          <td key={cls} className="py-2 px-3 font-bold border-r border-blue-100">
                      ...
                      <tr>
                        <td className="py-2 px-3 font-bold bg-emerald-50/50 border-r border-blue-100 text-emerald-900 text-left">
                          🌿 특수(통합)학생
                        </td>
                        {result.activeClassNames.map(cls => {
                          const c = (result.assignments[cls] || []).filter(s => s.통합학급).length;
                          return (
                            <td key={cls} className={`py-2 px-3 border-r border-blue-100 font-bold ${c > 0 ? 'bg-emerald-50 text-emerald-800' : 'text-slate-400'}`}>
                              {c > 0 ? `${c}명` : '-'}
                            </td>
                          );
                        })}
                      ...
                      <tr>
                        <td className="py-2 px-3 font-bold bg-orange-50/50 border-r border-blue-100 text-orange-900 text-left">
                          생활지도 '상'
                        </td>
                        {result.activeClassNames.map(cls => {
                          const c = (result.assignments[cls] || []).filter(s => s.생활지도 === '상').length;
                          return (
                            <td key={cls} className={`py-2 px-3 border-r border-blue-100 font-bold ${c > 0 ? 'bg-orange-50 text-orange-800' : 'text-slate-400'}`}>
                              {c > 0 ? `${c}명` : '-'}
                            </td>
                          );
                        })}
                      ...
                      <tr>
                        <td className="py-2 px-3 font-bold bg-amber-50/30 border-r border-blue-100 text-amber-900 text-left">
                          기초학력 부진
                        </td>
                        {result.activeClassNames.map(cls => {
                          const c = (result.assignments[cls] || []).filter(s => s.학습부진).length;
                          return (
                            <td key={cls} className={`py-2 px-3 border-r border-blue-100 font-bold ${c > 0 ? 'text-amber-800' : 'text-slate-400'}`}>
                              {c > 0 ? `${c}명` : '-'}
                            </td>
                          );
                        })}
                      ...
                      <tr>
                        <td className="py-2 px-3 font-bold bg-blue-50/30 border-r border-blue-100 text-blue-900 text-left">
                          운동부(학생선수)
                        </td>
                        {result.activeClassNames.map(cls => {
                          const c = (result.assignments[cls] || []).filter(s => s.학생선수).length;
                          return (
                            <td key={cls} className={`py-2 px-3 border-r border-blue-100 font-bold ${c > 0 ? 'text-blue-800' : 'text-slate-400'}`}>
                              {c > 0 ? `${c}명` : '-'}
                            </td>
                          );
                        })}
        ...
        <div className="p-4 bg-blue-50/50 border-t border-blue-100 flex flex-wrap justify-end gap-3">
          <button
            ...
            className="px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 flex items-center cursor-pointer"
          >
          ...
          <button
            ...
            className="px-4 py-2.5 bg-white hover:bg-blue-50 border border-blue-200 text-blue-700 rounded-xl text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 flex items-center cursor-pointer"
          >
